...
static void map_shared_lib_into_task(SharedLibrary* lib, Task* task) {
    ...
    uint32_t num_segments = header->e_phnum;
    for (uint32_t i = 0; i < num_segments; i++) {
        ...
        uint32_t addr = segment->p_vaddr + open_slib->offset;
        ...
        if (segment->p_flags & PF_W) {
            ...
            for (int j = 0; j < num_pages; j++) {
                uint32_t phys = mem_get_phys_from_virt(lib->elf.raw + segment->p_offset + j * 0x1000);
...
static void resolve_dynamic_references(ELFObject* elf, Task* task) {
    ...
    uint32_t* got = elf->got_section->sh_addr;
    uint32_t num_got_entries = elf->got_section->sh_size / elf->got_section->sh_entsize;

    for (int i = 3; i < num_got_entries; i++) {
        int rel_index = i - 3;

        assert(ELF32_R_TYPE(relocation_table[rel_index].r_info) == R_386_JUMP_SLOT);
        uint32_t sym_index = ELF32_R_SYM(relocation_table[rel_index].r_info);
        ...
        for (int s = 0; s < MAX_SHARED_LIBS_PER_TASK; s++) {
            ...
            if (found_sym != NULL) {
                uint32_t addr = found_sym->st_value + slib->offset;
                got[i] = addr;
                found = true;
                break;
            }
